...
tmeanMonthly <- t(extract(tave, county, fun=mean, df=TRUE, na.rm=TRUE))
tminMonthly <- t(extract(tmin, county, fun=mean, df=TRUE, na.rm=TRUE))
...
climoPlot<-ggplot() + 
  ...
  scale_y_continuous(
    ...
    sec.axis = sec_axis(~ . * maxP / maxT , name = "Precipitation (mm)"), 
    limits = c(0, 30)) + 
  theme_bw() + 
  theme(
