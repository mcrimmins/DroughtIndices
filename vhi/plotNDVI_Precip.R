...
pTSndvi<-ggplot(tsNDVI, aes(date, NDVI))+
  ...
  #ylim(0,1)+
  ggtitle(paste0("Monthly NDVI (1982-2018): ",countyName," County"))+
  labs(x = "Date",y="NDVI")
...
prec<-prec[[which(dates=="1982-01-01"):which(dates=="2015-12-01")]] 
# extract prec
tsPREC <- t(extract(prec, county, fun=mean, df=TRUE, na.rm=TRUE))
  tsPREC<-as.data.frame(tsPREC[c(-1),])
dates=as.data.frame(seq(as.Date("1982-01-01"), as.Date("2015-12-01"), by="month"))
  colnames(dates)<-"dateString"
tsPREC<-cbind(dates,tsPREC)
  ...
  tsPREC$prec<-(tsPREC$prec*days_in_month(tsPREC$date))/25.4
...
p2<-ggplot(precQuant, aes(x=date, y=avg)) +
    ...
    geom_line(size=.5)+
    scale_x_date(date_labels = "%B")+
    ggtitle(paste0("Monthly Precipitation (1982-2015): ",countyName," County"))+
...
pPrecip<-ggplot(yearlyPrec, aes(year, yearSumPrecip))+
  ...
  ggtitle(paste0("Total Annual Precipitation (1982-2015): ",countyName," County"))+
  labs(x = "Date",y="precip (in)")
...
pAllts<-plot_grid(pPrecip, pTSndvi, nrow = 2, align = "v")
