...
spi<-stack("/scratch/crimmins/livneh/processed/WESTmonthlyLivneh_SPEI6_1915_2015.grd")
# zonal stats
...
spiTSsd<-as.data.frame(rollapply(data = spiTS,width=20,FUN=sd, fill=NA))
spiTSsd$date<-seq(as.Date("1915-01-01"), as.Date("2015-12-31"), by="month")

# melt and use ggplot
library(reshape2)
library(ggplot2)
spiTSLong<-melt(spiTS,measure.vars = 1:12)
...
ggplot(spiTSLong, aes(x=date,y=value, fill=pos))+
  ...
  labs(x='month/year',y='SPI', title="Standardized Precip-ET Index - 6 month")+
  theme_bw()

spiTSsdLong<-melt(spiTSsd,measure.vars = 1:12)
ggplot(spiTSsdLong, aes(x=date, y=value))+
  geom_line()+
  facet_wrap(~ variable, ncol = 4, nrow = 3, strip.position="top")+
  labs(x='month/year',y='SPI SDev', title="SPEI-6 20yr Moving Std Dev")+
  theme_bw()
